...
# 1. Initialize model: 
#    model = F3KMEANS(n_clusters=10, type_balance='norm', type_init='random_c_kmeans', 
#                     type_update_centroids='shifted', max_iter=200, lambda_=0.5)
# 2. Fit data: 
...
class   F3KMEANS:
    ...
    def __init__(self, n_clusters,type_balance, type_init, type_update_centroids, max_iter=200, random_state=None, lambda_=0.5):
        ...
        self.type_of_update_centroids = type_update_centroids #'standard' / 'weighted' / 'inverse_force' / 'shifted'
    ...
    def update_centroids(self, X):
        ...
        for j in range(self.n_clusters):
            ...
            # === (1) Simple-weighted Centroid Update ===
            if self.type_of_update_centroids == "weighted":
            ...
            # === weighted_force_based ===
            elif self.type_of_update_centroids == "weighted_force_based":
                if(wi is None):
                    wi = self.compute_weights(X)
                #print(f"wi: {wi}")
                cluster_wi = wi[mask, j]
                #print(f"cl: {cluster_wi}")
                abs_w = np.abs(cluster_wi)
                #weights = 1.0 / np.where(abs_w == 0, epsilon, abs_w)
    ...
    def initialize_points(self, X, attributes):
        if self.type_init == 'random_centroids':
            ...
            #each centroid point take a label of its own cluster
    ...
    def compute_forces(self, X, centroids):
        ...
        norms = np.sum(diffs**2, axis=2)        # (n_samples, n_clusters)
    ...
    def fit(self, X, attributes):
        ...
        for _ in range(self.max_iter):
            iter +=1 
            new_distances = self.compute_forces(X, self.centroids)
            self.labels_ = self.random_argmax(new_distances)  # Randomly choose among
            self.find_clusters_r_b(self.attributes, self.labels_)
